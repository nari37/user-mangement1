...
const handleProfileChange = (e) => {
  ...
  setStudentdetails(prevState => ({
    ...
    const handleSubmit = (e) => {
        ...
        if (fileInputRef.current.files[0]) {
            ...
            axios.put(`http://localhost:8000/backend/student/updateProfilePicture/${id}`, formData, {
                headers: {
        ...
        axios.put(`http://localhost:8000/backend/student/updateProfilest/${id}`, {Studentdetails})
  ...
  return (
    ...
    {Studentdetails.profile ? (
        <img src={`http://localhost:8000/files/${Studentdetails.profile}`} alt="Profile" style={{ width: '5rem', height: '5rem', borderRadius: '50%', marginRight: '10px', marginBottom: '-30px' }} />
    ) : (
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', backgroundColor: 'rgb(28, 66, 95)', padding: '1rem', borderRadius: '0.5rem' }}>
        ...
        <input type='text' placeholder='Username...' style={{ padding: '0.5rem', border: '1px solid #ccc', borderRadius: '0.5rem', width: '100%' }} name='first_name' value={Studentdetails.first_name}  onChange={handleProfileChange} required />
        <input type='email' placeholder='Email...' style={{ padding: '0.5rem', border: '1px solid #ccc', borderRadius: '0.5rem', width: '100%' }} name='email' value={Studentdetails.email}  onChange={handleProfileChange} readOnly/>
